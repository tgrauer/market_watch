    ...
    <div class="row">
        <div class="col-xl-12 mt50">
            ...
            @if(!empty($data['company_profile']))
                <h2 class="comp_name">{{$data['company_profile']['company']['companyName']}} - {{$data['company_profile']['company']['symbol']}}</h2>
                <h3 class="current_stock_price">
                    @money($data['company_profile']['quote']['latestPrice'] * 100) 
                    <span class="latest_change {{$data['company_profile']['quote']['changePercent'] * 100 > 0 ? 'positive' : 'negative'}}">@money($data['company_profile']['quote']['change'] * 100) ({{round($data['company_profile']['quote']['changePercent'] * 100, 2)}}%) {!! $data['company_profile']['quote']['changePercent'] * 100 > 0 ? '<span class="positive"><i class="fa fa-arrow-up"></i></span>' : '<span class="negative"><i class="fa fa-arrow-down"></i></span>' !!}</span>
                </h3>
